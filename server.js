/**
 * PGB Gevonden landingspagina voor SamenOntzorgen.
 * Serveert gevonden.html en heeft een /contact-endpoint via nodemailer.
 *
...
// Pagina
app.get(['/', '/gevonden'], (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'gevonden.html'));
});
...
app.post('/contact', async (req, res) => {
  ...
  try {
    ...
    await transporter.sendMail({
      from: `"SamenOntzorgen PGB" <${emailUser}>`,
      to: mailTo,
      replyTo: email || undefined,
      subject: `Nieuwe aanmelding PGB van ${naam}`,
      html: `
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
          <div style="background:#1c2b2d;padding:22px;border-radius:8px 8px 0 0;">
            <h1 style="color:#fff;margin:0;font-size:19px;">Nieuwe aanmelding via PGB-gevonden pagina</h1>
          </div>
...
app.listen(PORT, () => {
  console.log(`PGB landingspagina draait op http://localhost:${PORT}`);
});
